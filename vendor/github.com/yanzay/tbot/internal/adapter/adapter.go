...
func (b *Bot) adaptUpdates(updates <-chan tgbotapi.Update, messages chan<- *model.Message) {
	...
	for update := range updates {
		...
		if updateMessage.From != nil {
			message.From = adaptUser(updateMessage.From)
		}
		...
		if update.CallbackQuery != nil {
			...
			if update.CallbackQuery.From != nil {
				message.CallbackQuery.From = adaptUser(update.CallbackQuery.From)
			}
		}
		switch {
		...
		case updateMessage.NewChatMembers != nil:
			message.Type = model.MessageNewMembers
			for _, u := range *updateMessage.NewChatMembers {
				message.Users = append(message.Users, adaptUser(&u))
			}
			messages <- message
...
func adaptUser(u *tgbotapi.User) model.User {
	if u == nil {
		return model.User{}
	}
	return model.User{
